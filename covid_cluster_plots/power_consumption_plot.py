...
def plot_violin(covid_val, non_covid_val, ax, title, ylabel):
    ...
    ax.text(x=0+0.48, y=int(get_max_pdf(covid_val)[1]), s="{:.2f}".format(get_max_pdf(covid_val)[0]), fontsize=15, color="black", va="center")
    ax.text(x=1+0.4, y=int(get_max_pdf(non_covid_val)[1]), s="{:.2f}".format(get_max_pdf(non_covid_val)[0]), fontsize=15, color="black", va="center")
